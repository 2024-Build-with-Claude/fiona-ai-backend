...
export class AuthController {
  ...
  async login(@User() user: UserWithSecrets, @Res({ passthrough: true }) response: Response) {
    ...
    Logger.log('Login response:', {
      ...
      cookies: {
        Authentication: (response.getHeader('Set-Cookie') as string[])?.find((cookie: string) => cookie.startsWith('Authentication=')),
        Refresh: (response.getHeader('Set-Cookie') as string[])?.find((cookie: string) => cookie.startsWith('Refresh='))
      }
    });
    return result;
  }
